"""Agentic Command Center — web dashboard (ADR-0018).
...
Read-only by construction (ADR-0018 SS3): this module must not import
...
The single inbound write path is `POST /operator` (ADR-0018 SS4): the browser
...
async def _ws_endpoint(websocket: WebSocket) -> None:
    ...
    dropped per ADR-0018 SS3 (GET /ws is a read-only push channel)."""
...
async def _operator_endpoint(request: Request) -> JSONResponse:
    """The single inbound write surface (ADR-0018 SS4). Accepts
